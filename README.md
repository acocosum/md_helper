...
除了知识库助手功能外，本项目还提供了杭州电子科技大学校园网自动登录程序，支持Windows系统集成、开机自启和定时检测。

### 功能特性

- **自动登录**: 基于HDU校园网登录接口，自动处理登录流程
- **状态检测**: 智能检测网络连通性，避免重复登录
- **Windows集成**: 支持任务计划程序和开机自启动
- **静默运行**: 后台运行，不打扰用户日常使用
- **日志记录**: 完整的操作日志，便于问题排查
- **配置管理**: JSON配置文件，密码本地加密存储
...
- **campus_login.py**: 主程序文件
- **config.json**: 配置文件（需要用户填写账号密码）
- **install.bat**: 一键安装脚本
- **uninstall.bat**: 卸载脚本
- **task_schedule.xml**: Windows任务计划配置
- **campus_login.log**: 运行日志文件

### 工作原理

1. **开机启动**: 系统启动1分钟后自动运行
2. **定时检测**: 每30分钟检查一次网络状态
3. **智能登录**: 仅在网络不通时才执行登录
4. **故障重试**: 登录失败时自动重试
5. **日志记录**: 所有操作都有详细日志
...
- 请确保Python环境已正确安装
- 配置文件中的密码将在本地进行MD5加密
- 程序仅在需要时进行登录，不会频繁访问服务器
- 如遇登录问题，请检查配置文件和日志文件

### 技术支持

如遇问题，请查看日志文件 `campus_login.log` 获取详细错误信息。
